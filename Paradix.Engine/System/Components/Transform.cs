...
namespace Paradix
{
	public sealed class Transform : Component
	{
		...
		{
			if (IsAttached && AttachedEntity.IsAttached && AttachedEntity.AttachedEntity.HasComponent<Transform> ()) 
			{
				var parentTransform = AttachedEntity.AttachedEntity.GetComponent<Transform> ();

				if (IsPositionRelative)
					_AbsolutePosition = (_RelativePosition.Rotate (parentTransform.AbsoluteRotation)) 
						* parentTransform.AbsoluteScale + parentTransform.AbsolutePosition;
				else
					_RelativePosition = (_AbsolutePosition.Rotate (-parentTransform.AbsoluteRotation)) 
						/ parentTransform.AbsoluteScale - parentTransform.AbsolutePosition;
